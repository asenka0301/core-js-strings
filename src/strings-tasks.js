...
function isPalindrome(str) {
  const formattedString = str.toLocaleLowerCase().replace(/[^a-z]/g, '');
...
function findLongestWord(sentence) {
  ...
  let theLongestWord = sentenceInArray[0];
  for (let i = 1; i < sentenceInArray.length; i += 1) {
    if (sentenceInArray[i].length > theLongestWord.length) {
      theLongestWord = sentenceInArray[i];
    }
  }
...
function reverseWords(str) {
  ...
  for (let i = 0; i < strInArray.length; i += 1) {
    result.push(strInArray[i].split('').reverse().join(''));
  }
  return result.join(' ');
}
...
function encodeToRot13(str) {
  ...
  const ROT13letters = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  for (let i = 0; i < str.length; i += 1) {
    const index = letters.indexOf(str[i]);
    if (index < 0) {
      encodedStr += str[i];
    } else {
      encodedStr += ROT13letters[index];
    }
  }
  return encodedStr;
}
